...
const { loadSources, loadRuntimeConfig, buildSnapshotFromSources } = require('../packages/adapters/src');
...
async function main() {
  ...
  console.log(`=== MODE: ${config.mode === 'ha' || config.ha.useLive ? 'home-assistant' : 'sample'} ===`);
  ...
  console.log('\n=== TELEGRAM WORKER RUN ===');
  console.log(await runDailyBrief(sources, { storageDir: stateDir }));
  console.log('\n=== FEEDBACK EVENT ===');
  ...
  console.log('\n=== HA PACKAGE BUILD ===');
  console.log(JSON.stringify(await buildHomeAssistantPackage(sources, { storageDir: stateDir }), null, 2));
}
